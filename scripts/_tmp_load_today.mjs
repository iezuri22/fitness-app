...
const dump = async (snap, label) => {
  ...
  for (const doc of snap.docs) {
    const w = doc.data();
    console.log(`\n--- ${doc.id} ---`);
    console.log(`slot=${w.slot} status=${w.status} title=${w.title} focus=${w.focus}`);
    const setsSnap = await db.collection(`users/${UID}/workouts/${doc.id}/sets`).orderBy('order').get();
    for (const s of setsSnap.docs) {
      const d = s.data();
      console.log(`  [${d.order}] ${d.exerciseName} | ${d.setType} | tgt=${d.targetReps}r@${d.targetWeight}lb act=${d.actualReps??'-'}r@${d.actualWeight??'-'}lb status=${d.status} notes=${d.userNotes||d.notes||''} completedAt=${d.completedAt?.toDate?.()?.toISOString?.()||'-'}`);
    }
  }
};
