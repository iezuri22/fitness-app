...
const dump = async (snap, label) => {
  ...
  for (const doc of snap.docs) {
    ...
    console.log(`  ${setsSnap.size} sets in subcollection:`);
    ...
    if (w.plannedSets && Array.isArray(w.plannedSets)) {
      console.log(`  ${w.plannedSets.length} plannedSets on doc:`);
      for (const d of w.plannedSets) {
        console.log(`  [${d.order}] ${d.exerciseName} | ${d.setType} | tgt=${d.targetReps}r@${d.targetWeight}lb`);
      }
    }
  }
};
